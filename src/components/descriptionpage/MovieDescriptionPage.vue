...
onMounted(async () => {
    try {
        const url = `https://api.themoviedb.org/3/movie/${route.fullPath.split("/")[2]}?language=en-US`;
        const urlcredit = `https://api.themoviedb.org/3/movie/${route.fullPath.split("/")[2]}/credits?language=en-US`;
        const options = {
            method: 'GET',
            headers: {
        ...
        const responseReview = await fetch(`http://localhost:5000/movies/${route.params.id}/?_embed=reviews`)
...
function crewFilter(job) {
    const data = infoCredits.value.crew?.filter(crew => crew.job == job)
...
<template>
    <div class="w-[100%] h-[100%]"
        :style="{ 'background-image': 'url(' + infoReview?.backdrop_path + ')', 'background-size': '100%' }">
        <div class="bg-layers h-[100%]">
            ...
            "English" : "IDK"
                        ...
                        <div>Director: <span class="font-medium">{{ crewFilter("Director") }}</span>
                        </div>
                        <div>Producer: <span class="font-medium">{{ crewFilter("Producer") }}</span>
                        </div>
                        <div>Release Data: <span class="font-medium">{{ infoDetails.release_date }}</span></div>
                        <div>Budget: <span class="font-medium">{{ revenueFormat(infoDetails.budget) }}</span></div>
                        <div>Box Office: <span class="font-medium">{{ revenueFormat(infoDetails.revenue) }}</span></div>
                        <div>Runtime: <span class="font-medium"> {{ timeFormat() }}</span></div>
                    </div>
                </div>
                <div class="Menu">
                    <div class="">
                        ...
                        <div class="my-[15px]">
                            <div class="ml-[auto] w-[100px]">
                                <button @click="handleShowAllCrew">
                                    <span class="hover:text-gray-400">{{ isShowAllCrew ? "Hide" : "Show All" }}</span>
                                </button>
                            </div>
                        </div>
                    </div>
...
.bg-layers {
    background: rgb(0, 0, 0);
    background: linear-gradient(180deg, rgba(0, 0, 0, 0) 0%, rgba(0, 0, 0, 0.15) 63%, rgba(0, 0, 0, 1) 83%);
}
