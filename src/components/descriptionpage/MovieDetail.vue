...
<template>
  ...
  <div class="flex justify-between mb-[20px]">
    <div class="w-[22%] mr-[3%] flex flex-col items-center">
      <img class="w-[225px]" :src="'https://image.tmdb.org/t/p/w500/' + movieDetails.poster_path" />
      <label for="my_modal_7"
        class="btn font-bold text-[22px] mt-[10px] pt-[2px] text-white font-istok gradient-bg border border-red-800 hover:opacity-70"
        @click="$emit('handleVideo')">🎞️ Trailer</label>
      <input type="checkbox" id="my_modal_7" class="modal-toggle" />
      <div class="modal" role="dialog" @click="$emit('handleVideo')">
        <div class="modal-box min-w-[200px] max-w-[1024px] h-[600px] p-0 flex items-center justify-center gradient-bg">
          <iframe v-if="isPlayVideo && movieDetails.trailer" class="w-[100%] h-[100%]" :src="'https://www.youtube.com/embed/' +
    ...
    <div class="w-[75%] ml-[8px] font-semibold">
      ...
      <div class="flex flex-wrap gap-[8px] items-center mb-[7px]">
        <div class="py-[4px] px-[15px] border-white border-2 rounded-[10px]" v-for="genere in movieDetails.genres"
          :key="genere">
          {{ genere.name }}
        </div>
      </div>
