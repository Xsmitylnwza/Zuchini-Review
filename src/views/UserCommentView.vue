...
import ListCommented from "@/components/commentpage/ListCommented.vue"
...
<template>
  ...
  <div v-if="dataLoaded" class="bg-gradient-to-r from-black to-red-900 min-h-screen">
    ...
    <div class="w-[80%] m-[auto] font-istok text-white px-24 py-8 bg-black bg-opacity-35 rounded-md">
      ...
      <ListCommented v-if="reviews.legth != 0" :items="reviews">
        <template #default="slotProps">
          <div class="w-[100%] h-[auto] min-h-[350px] flex mb-[30px] border-b-2 border-white">
            <div class="w-[25%] flex flex-col items-center">
              <CommentBanner :movie="movies[slotProps.key]" />
            </div>
            <div class="w-[75%] mt-[25px]">
              <CommentDetail :review="slotProps.item" :index="slotProps.key" @reviewModalHandler="reviewModalHandler"
